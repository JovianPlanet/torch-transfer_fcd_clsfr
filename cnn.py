...
class Cnn2D(nn.Module):
    ...
    def forward(self, x):
        ...
        # flatten the output from the previous layer and pass it through a set of FC => RELU layers
        x = torch.flatten(x, 1)
        x = self.fc1(x)
        x = self.relu3(x)
        ...
        # pass the output trough sigmoid classifier to get output predictions
